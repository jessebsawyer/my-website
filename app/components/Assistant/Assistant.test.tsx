...
describe('Assistant component', () => {
  it('renders intro and flips to chat UI on button click', () => {
    ...
    // Initial state
    expect(
      screen.getByRole('heading', { name: /hello, i'm jesse's ai assistant/i })
    ).toBeInTheDocument()
  ...
  it('shows initial assistant message in chat UI', () => {
    ...
    const allMatches = screen.getAllByText((content, node) =>
      Boolean(
        node?.textContent
          ?.toLowerCase()
          .includes(
            "ai: hi there! i'm jesse's personal assistant. ask me anything about his work, experience, or skills."
          )
      )
    )
  ...
  it('allows user to send a message and displays assistant reply', async () => {
    ...
    const userMessages = await screen.findAllByText((content, node) =>
    ...
    const aiReplies = await screen.findAllByText((content, node) =>
      Boolean(
        node?.textContent?.toLowerCase().includes('ai: this is a mock reply from the assistant.')
      )
    )
